...
func GetTourPackageByID(c *gin.Context) {
    ...
    if err := db.Preload("Province").Preload("TourPrices.PersonType").Preload("TourImages").Preload("TourDescriptions").Preload("Activities.Location").Preload("TourSchedules").First(&tourpackage, "id = ?", id).Error; err != nil {
